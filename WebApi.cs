...
public class WebApi : IWebApi
{
    ...
    {
        AddCustomHeaders(Client, customHeaders);
        var optionsResult = Client!.SendAsync(new HttpRequestMessage(HttpMethod.Options, optionsUri), cancellationToken)
    ...
    {
        AddCustomHeaders(Client, customHeaders);
        return await GetWebApiResultAsync<T>(
            await Client!.SendAsync(new HttpRequestMessage(HttpMethod.Options, optionsUri), cancellationToken));
    }
        ...
        string body, List<MailUser>? ccUsers, List<MailUser>? bccUsers, List<string>? filePaths)
    {
        ...
        // cc all ccUsers
        foreach (var ccAddress in ccAddressList) message.CC.Add(ccAddress);
        // bcc all ccUsers
        foreach (var bccAddress in bccAddressList) message.Bcc.Add(bccAddress);
